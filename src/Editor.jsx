...
export default function Editor({ projectId, onBack, user, theme, setTheme }) {
  ...
  useEffect(() => {
    ...
    setElements([]);
    pageHtmlRef.current = { 'page-1': '' };
    setActivePageId('page-1');
    setViewport({ x: 40, y: 34, zoom: 0.72 });
    setDocumentFont('excalifont');
    setLineHeight(1.68);
    setMode('canvas');
    ...
    supabase
      ...
      .then(({ data, error }) => {
        if (cancelled) return;
        if (error || !data) { setProjectReady(true); return; }
        setProjectName(data.name || '');
        const p = data.pages || initialPages;
        setPages(p);
        setElements(data.elements || []);
        pageHtmlRef.current = Object.fromEntries(p.map((page) => [page.id, page.html || '']));
        setActivePageId(data.active_page_id || p[0]?.id || 'page-1');
        setViewport(data.viewport || { x: 40, y: 34, zoom: 0.72 });
        setDocumentFont(data.document_font || 'excalifont');
        setLineHeight(data.line_height ?? 1.68);
        historyRef.current = [];
        futureRef.current = [];
        setMode('canvas');
        setSelectedElementIds([]);
  ...
  const onPointerMove = (event) => {
    ...
    if (drag.type === 'element') {
      ...
      setElements((items) => items.map((element) => {
        ...
        const nextY = Math.max(0, Math.min(PAGE_HEIGHT - Math.max(1, element.h || 1), element.y + dy));
  ...
  if (!projectReady) {
    return <div className="loading-screen"><p>Cargando proyecto...</p></div>;
  }
